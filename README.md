...
Comprehensive network performance monitoring toolkit for macOS. Collects Wi-Fi metrics, ping statistics (latency, jitter, loss), traceroute data, and public IP — then visualizes everything in an interactive HTML dashboard.

> **Platform**: macOS (Windows version planned)
...
| File | Description |
|------|-------------|
| `network_diag.sh` | Main diagnostics script. Collects Wi-Fi, IP, ping, traceroute, public IP. |
| `run_scheduled.sh` | Scheduler. Runs `network_diag.sh` every 5 minutes. Press Ctrl-C to stop. |
| `wifi_info.swift` | Swift helper using CoreWLAN to get unredacted Wi-Fi info on macOS. |
| `generate_dashboard.py` | Python script that reads `logs/summary.csv` and generates `logs/dashboard.html`. |
| `targets.txt` | Monitor targets, one per line. Supports domains and IPs. `#` for comments. |
...
Output written to `logs/YYYYMMDD/HHMMSS.txt` and `logs/summary.csv`.
